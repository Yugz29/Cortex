...
import { initDb, getLatestScans, getFunctions, cleanDeletedFiles, saveFeedback, getScoreHistory, getFeedbackHistory, saveTerminalError, getTerminalErrorHistory, updateTerminalErrorResolved, updateTerminalErrorLLM, getProjectScoreHistory, saveLlmReport, getLlmReport, getIntelMessages, saveIntelMessage, clearIntelMessages } from './database/db.js';
import { initMemoryTable, runMemoryEngineOnStartup, getMemories, getMemoriesForFile, dismissMemory, updateMemoryContent, extractMemoryFromAnalysis, extractMemoryFromError, buildMemorySnapshot } from './memory/memoryEngine.js';
import { askLLM, askLLMForError, askLLMProject, abortCurrentLLM } from './llm/llm.js';
import type { LLMContext, TerminalErrorContext, ProjectContext, IntelMessage } from './llm/llm.js';
...
let lastScoreSnapshot = new Map<string, number>(); // filePath → globalScore avant le scan
...
// Intercepte console.log pour les messages [Pulse] et les relaie au renderer
...
async function runScan(): Promise<void> {
    try {
        ...
        for (const file of result.files) {
            ...
            if (prev === undefined) continue; // nouveau fichier

            const delta = curr - prev;
            if (prev < 50 && curr >= 50) thresholdHit.push(name); // vient de passer critique
        ...
        if (thresholdHit.length > 0) {
            for (const name of thresholdHit) {
                emit('threshold', `${name} · crossed critical threshold`, 'critical');
            }
        }
        if (degraded.length > 0) {
            emit('degraded', `${degraded.slice(0, 2).join(', ')}${degraded.length > 2 ? ` +${degraded.length - 2} more` : ''} · score up`, 'warn');
        }
        if (improved.length > 0) {
            emit('improved', `${improved.slice(0, 2).join(', ')}${improved.length > 2 ? ` +${improved.length - 2} more` : ''} · score down`, 'ok');
        }

        const summary = totalDegraded > 0
            ? `${result.files.length} modules · ${totalDegraded} degraded${totalImproved > 0 ? ` · ${totalImproved} improved` : ''}`
            : totalImproved > 0
            ? `${result.files.length} modules · ${totalImproved} improved`
...
app.whenReady().then(async () => {
    ...
    // Notification directe dès réception socket — sans attendre le clipboard
    ...
    ipcMain.handle('pick-project', async () => {
        ...
        // Repointe le watcher sur le nouveau projet
    ...
    // ── Intel conversation persistence ──
    ipcMain.handle('get-intel-messages', () => getIntelMessages(getActiveProjectPath()));
    ipcMain.handle('save-intel-message', (_e, role: 'user' | 'assistant', content: string) =>
        saveIntelMessage(getActiveProjectPath(), role, content)
    );
        ...
        askLLM(
            ...
            () => {
                ...
                if (accumulated) {
                    ...
                    // Extraction mémoire silencieuse — fire-and-forget
                    extractMemoryFromAnalysis({
                        filePath:    ctx.filePath,
                        analysis:    accumulated,
                        projectPath,
                    }).then(() => {
                        mainWindow?.webContents.send('memories-updated');
                    }).catch(() => { /* silencieux */ });
                }
            },
    ...
    ipcMain.on('resolve-terminal-error', (_e, id: number, resolved: 1 | -1, errorContext?: { command: string; errorText: string; llmResponse?: string }) => {
        ...
        // Si résolu (pas ignoré) et qu'on a une analyse LLM — mémoriser la solution
        if (resolved === 1 && errorContext?.llmResponse) {
            extractMemoryFromError({
                command:     errorContext.command,
                errorText:   errorContext.errorText,
                llmAnalysis: errorContext.llmResponse,
    ...
    // Cleanup on quit (socket + clipboard — watcher géré plus bas)
    ...
    // Expose restart pour pick-project

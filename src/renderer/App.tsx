...
export default function App() {
  ...
  const [activeFilter,       setActiveFilter]       = useState<'critical' | 'stressed' | 'healthy' | 'imported' | null>(null);
  ...
  useEffect(() => {
    ...
    window.api.onMemoriesUpdated(() => window.api.getMemories().then(setMemories));
  ...
  function handleAnalyzeTerminalError() {
    ...
    const offDone  = window.api.onLLMDone(() => { setTerminalLlmLoading(false); offChunk(); offDone(); offError(); });
    const offError = window.api.onLLMError(err => { setTerminalLlm(`Error: ${err}`); setTerminalLlmLoading(false); offChunk(); offDone(); offError(); });
  ...
  // ── Markdown rendu une seule fois par valeur de terminalLlm ───────────────
    ...
    <div style={{ background: '#0a0a0b', color: '#d0d0d8', height: '100vh', display: 'flex', flexDirection: 'column', fontFamily: 'monospace', overflow: 'hidden', position: 'relative' }}>
      ...
      {terminalError && terminalError.mode === 'error' && (
        <TerminalErrorBanner
          ...
          onResolve={() => { if (terminalError) { window.api.resolveTerminalError(terminalError.id, 1, { command: terminalError.command, errorText: terminalError.errorText, llmResponse: terminalLlm || undefined }); setTerminalError(null); setTerminalLlm(''); } }}
        />
      )}
                      ...
                      <div style={{ fontSize: 10, lineHeight: 1.8, color: '#3a3a44' }}>
                        {line2parts.map((part, i) => {
                          ...
                          const color = isDegrading ? '#ef4444'
                                      ...
                                      : '#3a3a44';
                          const filter: typeof activeFilter = isImported ? 'imported' : null;
                          return (
                            <span key={i}>
                  ...
                  {activeFilter && (
                    <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                      <span style={{ fontSize: 9, color:
                        ...
                        activeFilter === 'healthy'  ? '#22c55e' : '#f97316',
            ...
            {(terminalLlm || terminalLlmLoading) ? (
              <div style={{ display: 'flex', flexDirection: 'column', height: '100%', fontFamily: 'monospace' }}>
                ...
                <div style={{ padding: '14px 16px', borderBottom: '1px solid #1e1e22', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <button onClick={() => { setTerminalLlm(''); setTerminalLlmLoading(false); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#4a4a52', fontSize: 16, padding: 0, lineHeight: 1 }}>×</button>
                </div>

...
| Geste                        | Effet                                                        |
|------------------------------|--------------------------------------------------------------|
| **Gâchette** (trigger)       | Raycast → sélection d'un nœud, surbrillance, panneau de métriques (score, complexité, cognitive, taille, churn, depth, params, fan in/out, hotspot). La sélection met aussi en évidence le **voisinage direct** : le nœud, ses fan-in/fan-out et leurs arêtes restent nets (arêtes accentuées), avec le **nom de fichier affiché au-dessus de chacun** ; les nœuds non connectés sont atténués et leurs arêtes entièrement masquées ; la désélection restaure tout. Le panneau apparaît en « pupitre » : ~0,55 m devant soi, sous la ligne de regard, face à l'utilisateur — posé à la sélection puis **fixe dans l'espace** (il ne suit ni le graphe ni la tête ; re-sélectionner un nœud le repose devant soi) |
...
| **Gâchette sur la barre sous le panneau** | Déplace le panneau (style fenêtre Quest) : il suit la main tant que la gâchette est tenue, puis reste où on le lâche. Ne sélectionne jamais de nœud. Fonctionne aussi quand le panneau est ancré |
| **Gâchette sur le bouton 📌 (à droite de la barre)** | Bascule l'ancrage du panneau. Gris = libre (chaque sélection replace le pupitre devant soi) ; vert = ancré (une sélection ne fait que rafraîchir le contenu, la pose ne bouge plus) |
